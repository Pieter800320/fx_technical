...
ARCHITECTURE:
  ...
  4. ADX weight  0.0-1.0  graduated, not a binary gate
  5. Structure   multiplier 0.40-1.30   H4/D1 only, via structure.py
                 BOS aligned  -> up to x1.30
                 CHOCH        -> down to x0.40
                 Conflict     -> x0.00  (alert killed, label = Conflict)
...
Keys returned — verified against scan_d1.py, scan_h4.py, scan_h1.py:
  score (int)  label  direction  filter_ok  filter_reasons
  raw (dict with adx)  signals
